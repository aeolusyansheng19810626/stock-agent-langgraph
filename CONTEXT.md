...
START → parse_node
  ...
  → 并行分析：scoring_node / risk_node / comparison_node / hypothesis_node / reflection_node
  → fan-in（5节点）→ report_node → END

* deep_read_node 条件触发（need_deep_read=True）：
  - use_financial_report=True 时，在 financial_report_node 之后运行，再进采集节点
  - ⚠️ deep_read_node 不在分析层 fan-in 里，它运行于采集节点之前，结果已在 state 中
```
...
- ❌ 新增分析节点后，不要把它加进 fan-in 列表（`add_edge([...], "report_node")`），除非它有从 data/news/rag 的无条件入边

### report_node prompt Token 优化约定
- `stock_data`：只传 `[技术面分析 by data_agent]` 部分，截掉 `[原始数据]` 以下内容
- `hypothesis_result`：只传 `conclusion` + `scenarios` 摘要，不要全量 `json.dumps`
- `chat_history_text`：截断到最近 12 行（约 3 轮），防止长对话 token 线性增长
...
- ✅ fan-in bug 修复（deep_read_node 错误加入 fan-in 导致 report_node 永远不触发）
- ✅ report_node prompt token 优化（截原始数据 / hypothesis 摘要 / history 截断，减少 30-40%）
- ✅ Step 卡片 UI 优化（llm 蓝色 / tool 绿色色标，args 截断 80 字符）
